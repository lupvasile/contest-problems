...
for(it=l.begin(),i=0;it!=l.end();++it,++i) linii.push_back(*it),ln[*it]=i;
for(it=c.begin(),i=0;it!=c.end();++it,++i) coloane.push_back(*it),cl[*it]=i;
    ...
    for(j=1;j<=coloane.size();++j)
    {
        ...
        else if(a[i][j]==maxx) nr+=1LL*(linii[i+1]-linii[i])*(coloane[j+1]-coloane[j]);
    }
